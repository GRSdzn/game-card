...
## Запуск демонстрационного боя

Приборы реагируют на действия: расход/восстановление Давления двигает стрелку
и выпускает пар; получение Защиты выдвигает бронепластины вокруг колбы ОЗ,
а её потеря убирает их с короткими искрами. Цифры обновляются сразу.
Завершение хода сопровождается отдачей кнопки, сигналом Инспектора и ударом
по колбе или щиту с видимым результатом попадания/блокирования.
Для просмотра цикла анимаций откройте
`scenes/debug/instrument_feedback_debug.tscn` и нажмите F6.

Откройте `project.godot` в Godot 4.7.x и нажмите **F5**. Теперь главная сцена
предлагает выбрать **Жадность** или **Одержимость**, затем запускает бой с
Инспектором. Наведите мышь на карту: прибор слева от ритуала покажет результат
её розыгрыша и немедленного завершения комбинации. ЛКМ разыгрывает карту.

Инспектор чередует атаки 5 и 11. Каждый завершённый ритуал снижает текущую
атаку 11 на 4, до минимума 3. После победы выберите одну компенсацию:
восстановление 8 ОЗ либо +4 к максимуму ОЗ с восстановлением 4 ОЗ.
Следующая смена сохраняет здоровье и порок. Новый забег сбрасывает награды.
Сохранения прохождения на диск пока нет.

Русский/английский переключается кнопкой `EN / РУ` на экранах выбора и через
архив во время боя. Подробные правила, формат ресурсов и команды проверки:
[`docs/DEMO_BATTLE.md`](docs/DEMO_BATTLE.md).
...
При давлении 3/3 доступны два клапана рядом с ритуалом: **стравить пар**
(1 давление → 4 защиты) или **перегрузить** (1 давление и 3 ОЗ → +8 урона
следующему ритуалу этого хода). Один режим за ход; неиспользованный заряд
сгорает. Прогноз учитывает бонус, кнопка предупреждает о смертельной плате.
Подробности и сцена проверки — в [демонстрационном бое](docs/DEMO_BATTLE.md).
